...
def load_weights_bias(module, layer: Layer):
    module.weight = nn.Parameter(
        torch.from_numpy(layer.WeightsAsNumpy()).reshape(
            tuple(layer.WeightsShapeAsNumpy())
        )
    )
...
def load_conv2d(layer: Layer) -> nn.Conv2d:
...
def load_batch_norm2d(layer: Layer) -> nn.BatchNorm2d:
    batch_norm = nn.BatchNorm2d(
        num_features=0, eps=layer.Eps(), momentum=layer.Momentum()
    )
...
def load_maxpool2d(layer: Layer) -> nn.MaxPool2d:
    kernel_size, stride = load_kernel_stride(layer)
